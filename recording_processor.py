...
class RecordingProcessor:
    ...
    def __init__(self, name: str, ts: datetime, transcript: str, chatbot: Chatbot) -> None:
        self.name = name
        self.ts = ts
    ...
    def process(self):
        ...
        for caption in webvtt.from_string(self.transcript):
            ...
            if speaker not in self.speaker_stats:
                self.speaker_stats[speaker] = {"num_words": 0, "num_turns": 0, "speaking_time": 0, "mean_word_speed": 0.0, "mean_words_per_turn": 0.0}
            content = fields[1]
            self.speaker_stats[speaker]["num_words"] += len(content.strip().split())
            self.speaker_stats[speaker]["speaking_time"] += (caption.end_in_seconds - caption.start_in_seconds)
